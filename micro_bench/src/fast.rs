...
impl MultisetHash for FastMultisetHash {
    ...
    fn from_compressed(digest: &[u8]) -> Self {
        ...
        current[0] = u64::from_be_bytes(digest[0..8].try_into().unwrap());
        current[1] = u64::from_be_bytes(digest[8..16].try_into().unwrap());
        current[2] = u64::from_be_bytes(digest[16..24].try_into().unwrap());
        current[3] = u64::from_be_bytes(digest[24..32].try_into().unwrap());
        FastMultisetHash { current }
    }
