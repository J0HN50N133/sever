...
impl MultisetHash for SecureMultisetHash {
    ...
    fn from_compressed(digest: &[u8]) -> Self {
        if digest.len() == 33 {
            let encoded_point = EncodedPoint::from_bytes(digest).unwrap();
            let point = ProjectivePoint::from_encoded_point(&encoded_point).unwrap();
            SecureMultisetHash { current: point }
        } else {
            // Return an empty hash if the digest is not a valid compressed point.
            SecureMultisetHash::new()
        }
    }
